...
mongoose.connect(
  process.env.MONGO_URI ||
    "mongodb://mongo:27017/music-library-watcher-aws-s3-mongo"
);
...
databaseService.addTrack = function (
  albumId,
  artistId,
  extension,
  s3key,
  ...
  return new Promise(function (resolve, reject) {
    if (albumId != null && artistId != null) {
      ...
      Track.find({ title: title })
        .where("albumId")
        .eq(albumId)
        .exec(function (err, track) {
          if (track.length > 1) {
            console.error(
              "Multiple tracks returned of the same name for same album"
            );
            reject();
          }
          ...
          if (track[0] == null) {
            ...
            console.log(
              "Track [" + track[0].toObject()._id + "] already exists, ignoring"
            );
      ...
      console.error(
        "An artistId or albumId is not present, cannot save track [" +
          s3key +
          "]"
      );
      reject();
